...
prompt = (
    ...
    '3. JSON格式必须标准规范，结尾不能有逗号\n'
    '4. 禁止以下行为：\n'
    '   - 推测性描述（如"可能"、"应该"等）\n'
    '   - 超出视觉证据的判断\n'
    '   - 修改JSON结构或键名'
),
@bp.route('/before_results', methods=['GET'])
def before_results():
    url = 'http://mofrp.top:10846/'
    API_KEY = 'sk-5ef7d974952841e08eb3d461bd759cc5'
    API_URL = 'https://dashscope.aliyuncs.com/compatible-mode/v1'
    model = 'qwen-vl-plus'
    urls=request.args.get('before_images')
    prompt = (
        '# 角色定位\n'
        '作为「居家养老服务质量审核专家」，需基于视觉证据严格验证服务真实性\n\n'
    ...
    if API_URL and API_KEY and model:
        response = get_api_inference(API_URL, API_KEY, model, prompt[0], urls)
    else:
...
if __name__ == '__main__':
    ...
    # MODEL = ""
    url = 'http://mofrp.top:10846/'
    start_time, error1_elder, error1_door, error_season = before_results(url, before_images, API_URL, API_KEY, MODEL)
